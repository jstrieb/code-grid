...
- Spreadsheet URLs are compressed by setting `<canvas>` element pixel data to
  the serialized sheet bytes, and having the browser convert the canvas to a PNG
  (since pixel data is ZLIB compressed in PNG files)
